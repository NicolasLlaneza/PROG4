...
import { useProductosQuery, useProductoMutations } from "../hooks/useProductos";
...
export default function ProductosAdminPage() {
  ...
  function handleGuardar(formData: ProductoFormData) {
    if (editando) {
      editar.mutate({ id: editando.id, data: formData }, { onSuccess: cerrarModal });
    } else {
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 md:p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mb-6">
            {productos.map((p: Producto) => (
              <ProductoCard key={p.id} producto={p} categorias={categorias} unidades={unidades} esAdmin={true}
                onEditar={(p) => { setEditando(p); setModalAbierto(true); }}
      ...
      {modalAbierto && (
        <ProductoFormModal
          inicial={editando ? { nombre: editando.nombre, descripcion: editando.descripcion, imagenUrl: editando.imagenUrl, precioBase: editando.precioBase, disponible: editando.disponible, unidadVentaId: editando.unidadVentaId, categoriaIds: editando.categoriaIds, ingredientes: editando.ingredientes } : null}

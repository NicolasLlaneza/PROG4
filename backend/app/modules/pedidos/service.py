...
class PedidoService:
    ...
    def _restaurar_stock_pedido(self, uow, pedido_id: int) -> None:
        """
        Al cancelar un pedido devolvemos al stock los ingredientes que se
        descontaron al crearlo. El stock del producto compuesto es derivado de
        sus ingredientes, asi que se recupera solo al recargar el producto.
        Para productos SIN ingredientes (insumos directos, ej: una bebida
        embotellada) restauramos su stock_cantidad almacenado.
        """
        detalles = uow.detalles.get_by_pedido(pedido_id)
        for detalle in detalles:
            ingredientes_del_producto = uow.producto_ingredientes.get_by_producto(detalle.producto_id)

            if ingredientes_del_producto:
    ...
    def create(self, data: PedidoCreate, usuario_id: int) -> PedidoConDetalle:
        with PedidoUnitOfWork(self._session) as uow:
            ...
            for item in data.items:
                ...
                pis_producto = uow.producto_ingredientes.get_by_producto(item.producto_id)

                # Stock check: si el producto tiene ingredientes lo derivamos
                # del stock de cada uno; si no, usamos su stock_cantidad manual.
                if pis_producto:
                    posibles = []
                    for pi in pis_producto:
                        ing = uow.ingredientes.get_by_id(pi.ingrediente_id)
                        if ing is None or pi.cantidad <= 0:
                            continue
                        posibles.append(int(ing.stock_cantidad // pi.cantidad))
                    stock_disponible = min(posibles) if posibles else 0
                else:
                ...
                # snapshot del precio actual (si el queso sube despues, este pedido conserva el precio original)
                ...
                # Descontamos stock. Si tiene ingredientes, solo descontamos
                # ingredientes (el stock del producto es derivado). Si no,
                # descontamos su stock_cantidad manual.
                if pis_producto:
    ...
    def list_mis_pedidos(self, usuario_id: int, offset: int, limit: int) -> PedidoList:
        ...
        return PedidoList(
            items=[PedidoPublic.model_validate(p) for p in pedidos],
            total=total,
            skip=offset,
            limit=limit,
        )
    ...
    def avanzar_estado(
        ...
        with PedidoUnitOfWork(self._session) as uow:
            ...
            # al cancelar devolvemos el stock que se habia descontado al crear
            if estado_hacia == "cancelado":
                self._restaurar_stock_pedido(uow, pedido.id)
